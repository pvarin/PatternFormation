...
function varargout = Chemotaxis1D(varargin)
    ...
    sol_index = 133;
    U0 = [U(:,sol_index); U(end:-1:1,sol_index)];
    V0 = [V(end:-1:1,sol_index); V(end:-1:1,sol_index)];
    
    %default simulation parameters
    dt = .01; numsteps = 110000;
    N = length(U0); dx = periods(sol_index)*2/N;
    
    %shift the IC so that the peaks will coalesce in the center
    U0 = U0([N/4+1:N,1:N/4]);
    V0 = V0([N/4+1:N,1:N/4]);
    
    %resample the data at 1:11
    r = 11;
    U0 = U0(1:r:end);
    V0 = V0(1:r:end);
    N = N/r;
...
%         dU0_dm = [U(:,sol_index-1); U(end:-1:1,sol_index-1)];
%         dV0_dm = [V(:,sol_index-1); V(end:-1:1,sol_index-1)];
...
%         epsV = (dV0_dm + dV0_dx).*cos(linspace(0,2*pi,N))';
        %uniformly distributed random data on (-1,1)
        epsU = .5*(rand(size(U0))*2-1);
        epsV = .5*(rand(size(U0))*2-1);
        %make the random data symmetric !!!IF YOU DO THIS YOU MISS THE MASS
        %INSTABILITY
%         epsU = epsU + epsU(end:-1:1);
%         epsV = epsV + epsV(end:-1:1);
        %don't add mass artificially
        epsU = epsU - mean(epsU);
        epsV = epsV - mean(epsV);
    U0 = U0 + epsU;
    V0 = V0 + epsV;
    ...
    opInvU = speye(N)-dt*Lap;
    opInvV = speye(N)-dt*(kappa*Lap);
