...
function varargout = Chemotaxis1D_implicit(varargin)
    %% Defaults
    ...
    [L_U, U_U, ~] = lu(speye(N)-dt*L);
    [L_V, U_V, ~] = lu(speye(N)-dt*(kappa*L-speye(N)));
    ...
    while nstep<=numsteps
        ...
        %implicit euler: linear diagonal terms
        U = U_U\(L_U\U);
        V = U_V\(L_V\V);
        
        %explicit euler: non linear and off-diagonal terms
        U = U - dt*b*D*(D*V.*U);
        V = V + dt*U;
    ...
    % Save Data
    if saveData
        save('Data/ImplicitEulerChemotaxisData_new.mat','U','V','x','t','dx','dt','b','kappa')
    end
    if saveSpeed
