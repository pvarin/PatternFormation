...
function varargout = Chemotaxis2D_logistic(varargin)
    %% Defaults
...
%         U0(:,:) = U0(:,:) + .2*(rand(size(U0))-.5);
    ...
    if ~V0init
        V0 = ones(M,N)*.3;
%         V0(floor(end/2),1) = V0(floor(end/2),1) + .1;
%         V0(:,1) = V0(:,1) + .1*(rand(size(V0(:,1)))-.5);
    end
    ...
    Dx = derivative(dx,N,'symmetric','periodic'); Dy = derivative(dy,M,'symmetric','periodic');
    Lx = laplacian(dx,N,'periodic'); Ly = laplacian(dy,M,'periodic');
...
%     Diffuse_Vy = speye(M)+dt/2*kappa*Ly;
    ...
    while nstep<=numsteps
        ...
        U = U - dt*(chi*((V*Dx').*U.*(1-U))*Dx');
        U = U - dt*(chi*Dy*((Dy*V).*U.*(1-U)));
